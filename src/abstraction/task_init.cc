...
void Task::init() {
	...
	Task * t = new (SYSTEM) Task(new (SYSTEM) Address_Space(MMU::current()),
			new (SYSTEM) Segment(Log_Addr(si->lm.app_code),
					si->lm.app_code_size),
			new (SYSTEM) Segment(Log_Addr(si->lm.app_data),
					si->lm.app_data_size),
	...
	if (si->bm.extras_offset != -1)
		db<Init>(INF) << "Task:: additional tasks at "
				<< reinterpret_cast<void *>(si->pmm.ext_base) << ":"
				<< si->pmm.ext_top << endl;
