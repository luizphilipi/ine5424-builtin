...
// Methods
Address_Space::Address_Space(MMU::Page_Directory * pd) :
		MMU::Directory(pd) {
	db<Address_Space>(TRC) << "Address_Space(pd=" << pd << ") [Directory::pd="
			<< Directory::pd() << "] => " << this << endl;
}
...
Address_Space::~Address_Space() {
	db<Address_Space>(TRC) << "~Address_Space(this=" << this
			<< ") [Directory::pd=" << Directory::pd() << "]" << endl;
}
...
Address_Space::Log_Addr Address_Space::attach(const Segment & seg,
	...
	db<Address_Space>(TRC) << "Address_Space::attach(seg=" << &seg << ",addr="
			<< addr << ") => " << tmp << endl;
...
Address_Space::Phy_Addr Address_Space::physical(
